...
public class Crawler {
	...
	public TOS crawl(String initialUrlString) throws InvalidURLException, DownloadException {
		...
		// Create URL object
		URL initialUrl = urlMod.create(initialUrlString);
		
		// BEGIN Check for existing entry in the DB
		tos = TOS.loadByURL(initialUrl.toString());
		if(tos != null){
		...
		if(old){
			if(logging || debug) System.out.println("Archived old version (" + tos.getVisitedOn() + ")");
			tos.archive();
			tos.dropPages();
		}
		...
		if(tos == null || old){
			...
			String domain = urlMod.getDomain(initialUrl.toString());
			if(logging) System.out.println("Domain: " + domain);
			queue.add(initialUrl.toString(), 100);
			...
			while(!queue.empty() && !end){
				...
				if(urlString.contains(domain)){
					...
					if(url!= null && !visited.contains(url.toString())){
						visited.add(urlString);
						
						if(downloader.checkRobots(url)){
							if(logging) System.out.print(" -  ");
						} else {
							if(logging) System.out.print(" !  ");
						}
						
						if(debug || logging) System.out.println(urlString);
							...
							// ELSE If it's 302 or 301
								...
								if(file != null){
									...
									if(found){
										...
										if(logging) System.out.println(" -> " + url.toString());
										
										if(tos == null){
											tos = new TOS();
											tos.setName(file.filename());
											tos.setUrl(initialUrl.toString());
										}
		...
		if(tos != null){
			// Save entry
			tos.save();
			System.out.println(tos.getPages().size() + " pages");
		}
